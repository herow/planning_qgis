 ...
 * file in the same directory with the same basename + â€?xsdâ€? This xsd file must be
 ...
 *
 * In the case of a HTTP URL the â€˜FILTERâ€?query string parameter can be used to filter
 * the WFS feature type. The â€˜FILTERâ€?key value can either be a QGIS expression
 ...
 * QGIS expression filter only supportsâ€?=, !=, <, >, <=, >=, AND, OR, NOT, LIKE, IS NULLâ€?
 * attribute operators, â€œBBOX, Disjoint, Intersects, Touches, Crosses, Contains, Overlaps, Withinâ€?
 * spatial binary operators and the QGIS local â€œgeomFromWKT, geomFromGMLâ€?
